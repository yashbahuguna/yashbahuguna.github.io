...
document.getElementById("assign").onclick = () => {
        if (onoffstate == 1) {
                delta = Number((Math.random()*3.14).toPrecision(2));
                d3.select('.locus').remove();
                addFigure();
        }
...
document.getElementById("checkAnswer").onclick = ()=>{
        ...
        error = ((enteredValue-delta)*100/delta).toPrecision(2)
        if (onoffstate == 1) {
                if (A != B) {
                        document.getElementById("result").innerHTML = "amplitudes are not equal";
                }
                else if (a != b) {
                        document.getElementById("result").innerHTML = "frequencies are not equal";
                }
                else if (error >10) {
...
document.getElementById("enterReading").onclick = ()=> {
    ...
    var newCellAmplitude  = newRow.insertCell(1);
    var newCellFrequency  = newRow.insertCell(2);
    var newCellCalculated  = newRow.insertCell(3);
    var newCellActual  = newRow.insertCell(4);
    var newCellError  = newRow.insertCell(5);

    newCellsno.innerHTML = sno.toString();
    newCellAmplitude.innerHTML = A.toString();
    newCellFrequency.innerHTML = a.toString();
    
    newCellCalculated.innerHTML = enteredValue.toString();
    newCellActual.innerHTML = delta.toString();
